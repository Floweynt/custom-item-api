...
public class EnchantComponent implements TaggedItemComponent {
    ...
    public void putComponentsStart(Consumer<Component> output) {
        config.enchants.stream().map(enchant -> {
            final var main = Component.text(enchant.enchant().displayText).color(NamedTextColor.GRAY);
            if (enchant.enchant.shouldDisplayLevel)
                return main.append(Component.space())
                    .append(Component.text(Utils.toRoman(enchant.level)).color(NamedTextColor.GRAY));
            return main;
        }).forEach(output);
    }
    ...
    public enum Enchantment {
        ...
        private static final Map<String, Enchantment> TABLE = Arrays.stream(values()).collect(Collectors.toMap(
            e -> e.id,
            e -> e
        ));
        ...
        Enchantment(String id, String displayText, boolean displayLevel) {
            this.id = id;
            this.displayText = displayText;
            this.shouldDisplayLevel = displayLevel;
        }

        public static Enchantment fromJson(JsonElement e) {
    ...
    private record EnchantmentInstance(Enchantment enchant, int level) {
        public static EnchantmentInstance fromJson(JsonElement e) {
            return new EnchantmentInstance(
                Enchantment.fromJson(e.getAsJsonArray().get(0)),
